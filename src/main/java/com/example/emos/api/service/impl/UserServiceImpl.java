...
//    private String code;
    ...
    public String searchByIdPassword(int userId) {
        return userDao.searchByIdPassword(userId);
    }
    ...
    public PageUtils searchUserByPage(HashMap param) {
        ArrayList<HashMap> list = userDao.searchUserByPage(param);//获取分页数据
        long count = userDao.searchUserCount(param);//获取符合条件的总数
        int start = (Integer) param.get("start");//起始页
        int length = (Integer) param.get("length");//查询的条数
        PageUtils pageUtils = new PageUtils(list, count - 1, start, length);//封装数据进行返回
        return pageUtils;
    }
